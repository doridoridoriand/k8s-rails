...
起点となった ice-juice-immerse の `K8sService` には、公式 `kruby` クライアントを
...
| K2 | kruby はシンボルキーの Hash を返す。Rails 側（JSON/ビュー）では文字列キーで扱うため、`deep_stringify_keys` による統一が必須 |
| K3 | クラスタが到達不能な場合、呼び出し側（ビュー等）が生の `kruby` 例外を扱うと 500 になる。接続不能 / リソース不在 / API エラーを**gem 側の例外体系**で格納し、Rails 側はそれを素通し表示できる規約が必要 |
| K4 | read-only（get/list）と操作（create/patch/update）は障害時の影響度が異なるため、**フェーズごとに API を分離**する運用（immerse の M2 read-only → M3 操作）が事実上のベストプラクティス |
...
K8s CRD を扱えるようにし、(b) immerse 自体をこの gem に移行して両方向で検証する。
...
- v0.1 / v0.2 のリリース境界
- immerse への移行手順（v0.1 検証の受け皿）

### 2.2 非スコープ（本設計の外）

- core v1 リソース（Pod / Deployment / Service 等）のフルサポート — CRD 中心の gem。core v1 は「group=""/version="v1"」の CRD 宣言で代替可能だが、v0.1 では対象外
...
| P2 | クライアントは `kruby`（公式 OpenAPI クライアント系）。immerse と同様に `~> 1.36` を基本 pin |
| P3 | 認証は (a) in-cluster ServiceAccount、(b) KUBECONFIG の 2 パターンのみを扱う。exec plugin / 他方式は v0.1 で保証しない |
| P4 | immerse の K8s 利用（Argo Workflows / CronWorkflow / CrawlProgress CR）が、本 gem の移行検証の**最初かつ最低限のユースケース**である |
...
└──────────────────────────┴────────────────┼────────┘
                           │                │
┌──────────────────────────┴────────────────┴────────┐
│ kruby (~> 1.36) → Kubernetes API Server             │
...
└── examples/
    └── rails-app/           # 最小 Rails 7.1 例（immerse 移行の雛形兼用）
```
...
KubeRails.configure do |config|
  config.namespace = ENV.fetch("KOKKAI_DIVE_K8S_NAMESPACE", "doridoridoriand")
...
| `connection` | `nil`（自動検出） | `Kubernetes::Configuration` インスタンス。テストではスタブ接続をここから注入する想定 |
...
3. `Kubernetes::ApiClient` → `Kubernetes::CustomObjectsApi` を生成し、**文字列キー化**（`deep_stringify_keys`、K2）を API レスポンス後に行う
4. 接続・認可の失敗は `KubeRails::Unavailable` に変換して throw（リトライはしない）
...
宣言で生成されるメソッド（全て class メソッド。`namespace` 引数は `resource.namespace` で上書き可）:
...
- **戻り値は常に文字列キーの Hash**（K2 の規約を API 契約として固定）。
  `find` は存在しない場合 `KubeRails::NotFound` を raise（immerse が `return nil` にしていたのは
...
| kruby 側 | → gem 側 |
|---|---|
| 接続・DNS・タイムアウト系（`StandardError` で `ApiError` 以外） | `Unavailable` |
...
begin
  ...
  render "k8s_unavailable"        # immerse の「K8s 未接続」バナー相当
...
| 依存 | 制約 | 理由 |
|---|---|---|
| Ruby | `>= 3.2` | immerse が 3.3.8 実行。gem として広く使う 3.2 を下限に |
| `kruby` | `~> 1.36` | immerse と同一 pin。1.36 系で K1 橋渡しが確定している。kruby 1.37+ で API が変われば Client 側を 1 箇所だけ修正する設計にする（§7） |
| `activesupport` | **任意**（`>= 7.0`） | `defined?(ActiveSupport::Notifications)` でガード。Rails 無し環境（Cron スクリプト等）でも動作する必要がある |
...
| レイヤー | 手法 | 対象 |
|---|---|---|
| ユニット | `KubeRails.config.connection` に**スタブ ApiClient**（kruby のメソッドをモックした素のオブジェクト）を注入 | client（橋渡し・例外変換）、resource（整形・readonly 制限）、crd（メソッド生成） |
| 設定 | spec 間で `KubeRails.reset!` | 宣言の破棄・再接続 |
| 集積（任意） | GitHub Actions で **kind**（または既存 microk8s に接続するジョブ）で実クラスタ E2E | v0.1 の必須ではない。**推奨**: immerse 移行時の検証を兼ねる |

- 本設計では CI は `rspec` + `rubocop` のみを必須とし、kind E2E は v0.2 以降で
  GitHub Actions の追加として扱う（自宅 microk8s への接続 CI はネットワーク依存のため採用しない）。

## 10. リリース計画

| バージョン | 内容 | 出口基準 |
|---|---|---|
| **v0.1** | §5 の公開 API（CRD 宣言 / list / find / create / patch / 例外 / 計測 / スタブテスト）+ README | rspec 全緑 + **immerse の `K8sService` を `kuberails` に移行して動作確認**（§12） |
| v0.2 | watch（`watch` メソッド、kruby の watch サポート上）、core v1 の group="" 宣言サポート、kind E2E の CI 化 | v0.1 運用 1 ヶ月後のフィードバック |
...
- **watch**: kruby の watch はストリーム処理であり、Rails のリクエスト応答型には不向き。
  ...
  形で、ポーリング置き換えのユースケース（immerse ダッシュボードの 30 秒ポーリング）から設計する。
...
## 12. immerse への移行（v0.1 検証）

immerse の `app/services/k8s_service.rb`（168 行）を `kuberails` に置き換える:

1. `Gemfile` に `gem "kuberails", path: "../kuberails"`（開発期間限定）
2. initializer に CRD 宣言（Workflow / CronWorkflow / CrawlProgress の 3 種、
   `readonly: false`（M3 操作系が既にあるため））
3. `K8sService` 内を `KubeRails.client` 呼び出しに置換。**整形メソッド
   （workflow_summary / crawl_progress_summary / summarize_steps）は immerse 側に残す**
   （アプリ固有の表示ロジックのため、gem には載せない）
4. 例外: immerse の `K8sService::Unavailable` は `KubeRails::Unavailable` に alias/
   rescue 統一
5. 検証: `make test`（immerse 側）+ ダッシュボードの K8s 読取が KUBECONFIG 経由で
   従来通り表示されること（自宅 microk8s への手動確認）

移行後も `K8sService` を**整形ラッパーとして残す**（コントローラの呼び出し先を変えない、
PR の差分を最小化）。
